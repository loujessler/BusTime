...
from bot.handlers.main.inline_handlers.back_to_main_menu import handler_back
from bot.handlers.main.bot_start import edit_ls
from bot.utils.localization.i18n import MessageFormatter


@dp.callback_query_handler(text='delete_bus_stop')
async def choose_stop(call: types.CallbackQuery):
    ...
    ikb.inline_keyboard = ikb_menu_bus_stops(
        user,
        bus_stops,
        False).inline_keyboard + ikb_default(user.language,
...
@dp.callback_query_handler(state=Regist.delete_bus_stop)
async def handler_delete_bus_stop(call: types.CallbackQuery, state: FSMContext):
    ...
    if delete_stop:
        await edit_ls.edit_last_message(
            ...
            call, ikb_default(user.language, {'back_to_settings': 'back_to_settings',
                                              'back_to_main_menu': 'back_to_main_menu'})
        )
    else:
        await edit_ls.edit_last_message(
